...
void loop() {
  ...
  if (digitalRead(2) == LOW) {
    ...
    for (byte number = 0; number < sizeofSendingRollingCode; number++) {
      ...
      Serial.print(rollingCode[rollingCodeNumber + number]); Serial.print(", ");
  ...
  while (preambleCounter < sizeofPreamble) {
    if (digitalRead(2)) {
      ...
      Serial.print("Preamble = "); Serial.println(preambleCounter);
    }
  }

  //// TODO - check this code, ReadDataPins() should handle the timing...
  //pulseTimer = millis() + 500;
  //while (!digitalRead(2) && millis() < pulseTimer);
  //if (millis() >= pulseTimer) {
  //  Serial.println("Failed to capture Rolling Number");
  //  goto FAIL;
  //}

  // read the rolling code
  Serial.print("sizeofSendingRollingCode: "); Serial.println(sizeofSendingRollingCode);
  Serial.print("Rolling Code: ");
  byte sendingRollingCodeCounter = 0;
  while (sendingRollingCodeCounter < sizeofSendingRollingCode) {
    if (digitalRead(2)) {
      readRollingCode[sendingRollingCodeCounter] = ReadDataPins();
      Serial.print(readRollingCode[sendingRollingCodeCounter]); Serial.print(", ");
  ...
  int tryRollingNumber = 0;
  //byte readRollingNumberCounter = 0;
  passed = 0;

  // check next 5 rolling codes
  for (int i = 0; i < consecutiveChecks * sizeofSendingRollingCode; i = i + sizeofSendingRollingCode) {
    //Serial.print("Start: tryRollingNumber = "); Serial.println(tryRollingNumber);
    //Serial.print("Start: rollingCodeNumber = "); Serial.println(rollingCodeNumber);
    //Serial.print("Start: for / next i = "); Serial.println(i);
    
    tryRollingNumber = rollingCodeNumber + i;
    //Serial.print("Running: tryRollingNumber = "); Serial.println(tryRollingNumber);
    //Serial.print("Running: rollingCodeNumber = "); Serial.println(rollingCodeNumber);
    //Serial.print("Running: for / next i = "); Serial.println(i);

    if (tryRollingNumber >= sizeofRollingCode) tryRollingNumber = 0;
    // readRollingNumberCounter = 0;
    for (int x = tryRollingNumber; x < tryRollingNumber + sizeofSendingRollingCode; x++) {
      //Serial.print("Running: for / next x = "); Serial.println(x);
      if (readRollingCode[x - tryRollingNumber] == rollingCode[x]) {
        Serial.print("Try rolling Code Position "); Serial.println(x);
        Serial.print(" *** Rolling Code Correct *** "); Serial.print(readRollingCode[x - tryRollingNumber]); Serial.print(" = "); Serial.println(rollingCode[x]);
        passed++;
      }
      else {
        Serial.print("Try rolling Code Position "); Serial.println(x);
        Serial.print(" Rolling Code INCORRECT "); Serial.print(readRollingCode[x- tryRollingNumber]); Serial.print(" <> "); Serial.println(rollingCode[x]);
        passed = 0;
      }
    }
  ...
  Serial.println(" *** Passed Rolling Code Checks *** ");
