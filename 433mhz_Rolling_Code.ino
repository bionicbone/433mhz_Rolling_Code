...
uint8_t TxIdNumber = 1;                     // user should set this (when flashing Tx) to 0, 1, or 2 so Rx can determine which Tx is being used.
...
void TransmitData(uint8_t btnNumber, uint8_t DataArray[], uint8_t sizeofDataArray) {
  ...
  uint8_t functionButtons = 0;
  uint8_t tempRollingCodeNumber = rollingCodeNumber[TxIdNumber];
